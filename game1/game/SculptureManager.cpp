...
void SculptureManager::setPiecePosition( int inPieceHandle,
    ...
    Vector3D *currentPosition = mCurrentPiecePositions[ inPieceHandle ];

    currentPosition->mX = inNewPosition->mX;
    currentPosition->mY = inNewPosition->mY;
    currentPosition->mZ = inNewPosition->mZ;
...
char SculptureManager::isPieceJarred( int inPieceHandle ) {
    if( mCurrentJarForces[ inPieceHandle ] > 0 ) {
        return true;
        }
    else {
        return false;
