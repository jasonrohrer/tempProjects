...
GameSceneHandler::GameSceneHandler( int inStartingLevel )
    : mLevelNumber( inStartingLevel - 1 ),
      mFadeOutLevel( 0 ),   // start fully faded out,
...
void GameSceneHandler::drawScene() {
    ...
    */
    ...
    double shipParameter = ( ( mMoveRate / mMaxMoveRate ) / 2 ) + 0.5;
    ...
    if( mFadeOutLevel < 1 ) {
        ...
        glBegin( GL_QUADS );
            glColor4f( 0, 0, 0, 1 - mFadeOutLevel );

            glVertex2d( 2 * mMinXPosition, 2 * mMinYPosition );
            glVertex2d( 2 * mMinXPosition, 2 * mMaxYPosition );
            glVertex2d( 2 * mMaxXPosition, 2 * mMaxYPosition );
            glVertex2d( 2 * mMaxXPosition, 2 * mMinYPosition );
...
void GameSceneHandler::fireRedraw() {
    ...
    if( mMovingUp ) {
        mMoveRate += mShipAccelleration * frameSecondsDelta;
        if( mMoveRate > mMaxMoveRate ) {
            mMoveRate = mMaxMoveRate;
            }
    ...
    moveVector->mY += mMoveRate;
    
    if( mMovingLeft ) {
        moveVector->mX += 2 * frameSecondsDelta;
        }
    if( mMovingRight ) {
        moveVector->mX -= 2 * frameSecondsDelta;
        }
    ...
    if( !mMovingUp && mMoveRate > 0 ) {
        mMoveRate -= mShipFriction * frameSecondsDelta;
    ...
    viewPosition->mZ =
        baseViewZ -
        40 * fabs( mMoveRate ) / mMaxMoveRate; 
    ...
    // handle the fade-out or fade in
    if( mBossManager->isBossDead() && mFadeOutLevel > 0 ) {
        // fade out more
        double fadeDelta = frameSecondsDelta / mFadeTime;

        mFadeOutLevel -= fadeDelta;
        if( mFadeOutLevel < 0 ) {
            mFadeOutLevel = 0;
            }
        ...
        // this level's music
        mSoundPlayer->setMusicLoudness( mMusicLoudness * mFadeOutLevel );
        }
    else if( ! mBossManager->isBossDead() && mFadeOutLevel < 1 ) {
        // fade in more
        double fadeDelta = frameSecondsDelta / mFadeTime;
